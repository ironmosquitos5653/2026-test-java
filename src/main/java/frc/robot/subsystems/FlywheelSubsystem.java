...
public class FlywheelSubsystem extends SubsystemBase {
  ...
  private final int canId = 35;
  private final SparkFlex flyWheelMotor;
  private final RelativeEncoder flyWheelEncoder;

  /** Creates a new FlywheelSubsystem. */
  public FlywheelSubsystem() {
    flyWheelMotor = new SparkFlex(canId, MotorType.kBrushless);
...
private double kP = 0.0005;      // example: 0.0005 percent-per-RPM (tune)
private double kFF = 1.0 / 5676; // example feed-forward: fraction-per-RPM (for NEO maxRPM=5676). Use 12/5676 if using volts-per-RPM.
private final boolean useVoltageControl = false; // true -> use setVoltage, false -> use percent output

// Add if you need to scale the kP value differently; tune carefully
private final double outputDeadband = 0.0; // leave 0 or set small minimum to overcome static friction
...
public void periodic() {
    ...
    // If using voltage control with motor.setVoltage(), kP would be volts-per-RPM and kFF volts-per-RPM
    double output;
