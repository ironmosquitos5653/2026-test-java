...
/**
 * Small helper controller that computes an angular velocity to rotate the robot so it faces a
 * target pose.
 *
 * <p>Usage: - Construct with tuned PID gains. - Call calculate(currentPose, targetPose) each loop
 * to get angular velocity (rad/s). - Optionally call atSetpoint() to know when the rotation is
 * within tolerance.
 */
public class TurnToPoseController {
  ...
  /**
   * Set the maximum absolute angular velocity (rad/s) this controller will output. If not set, the
   * controller will not clamp its output.
   */
  ...
  /**
   * Calculate the angular velocity (rad/s) to rotate from currentPose to face the targetPose. The
   * sign follows the usual rotation convention: positive means counter-clockwise.
   */
